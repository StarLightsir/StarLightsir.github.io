...
chuangzuoren:'',
    ...
    methods: {
	  ...
	  getDetails(id){
			this.$axios
                .get("/api/front/getlepuxinxidetail/"+id,)
                .then(res => {
                  this.lepuxinxi = res.data.data;
                  this.editForm.lepubianhao = this.lepuxinxi.lepubianhao;
					this.editForm.lepumingcheng = this.lepuxinxi.lepumingcheng;
					this.editForm.lepuleibie = this.lepuxinxi.lepufenlei;
        ...
        this.$refs[editData].validate(valid => {
          if (valid) {
            if(this.editForm.id){
              ...
              this.$axios
                      .post("/api/front/postgoumai/",this.editForm)
                      .then(res => {
                        if (res.data.code == '0') {
						//kuabiaogaizhi1
                          this.$message({
