    ...
    <el-dialog title="注册" width="300px" center :visible.sync="isRegister">
      <el-form :model="RegisterUser" :rules="rules" status-icon ref="ruleForm" class="demo-ruleForm">
        <el-form-item prop="role">
          <el-radio v-model='RegisterUser.role' label='爱好者'>爱好者</el-radio><el-radio v-model='RegisterUser.role' label='创作者'>创作者</el-radio>
        </el-form-item>
      ...
      if (userNameRule.test(value)) {
        //判断数据库中是否已经存在该用户名
		 let bb=this.RegisterUser.role
        	if (bb === '爱好者') {
                        this.$axios
                            .post("/api/front/findaihaozheyonghuming", {yonghuming: this.RegisterUser.name})
                            .then(res => {
                                // “001”代表用户名不存在，可以注册
                                if (res.data.code == "0") {
                        ...
                        this.$axios
                            .post("/api/front/findchuangzuozhezhanghao", {zhanghao: this.RegisterUser.name})
                            .then(res => {
                                // “001”代表用户名不存在，可以注册
                                if (res.data.code == "0") {
                                    this.$refs.ruleForm.validateField("checkPass");
    ...
    Register() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        ...
        if (valid) {
            ...
            this.$axios
                    .post("/api/front/zhucechuangzuozhe", {
