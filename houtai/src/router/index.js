...
export function aihaozheUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuAihaozhe()
		.then(async (res) => {
		...
		.catch(() => {});
}export function chuangzuozheUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuChuangzuozhe()
		.then(async (res) => {
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'aihaozhe') aihaozheUser(router, to, next);if (Session.get('userInfo').roles[0] === 'chuangzuozhe') chuangzuozheUser(router, to, next);
}
