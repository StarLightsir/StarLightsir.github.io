		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="danger" @click="deleteAihaozhe(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan' ">删除</el-button>
...
import { aihaozheList, aihaozheSave, aihaozheDelete,aihaozheEdit,aihaozheDeleteList } from '@/api/aihaozhe/aihaozheApi';
...
export default {
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const aihaozhes = this.aihaozhes;
				if(aihaozhes.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						aihaozhes.forEach(item=> {
							let data = {...item, issh: 1}
							aihaozheEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		// 删除公司
		deleteAihaozhe(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				aihaozheDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '爱好者已删除!'
		...
		handleDeleteList(){
			...
			if(aihaozhes.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					aihaozheDeleteList(aihaozhes).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
