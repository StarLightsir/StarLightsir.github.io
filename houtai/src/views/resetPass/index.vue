...
<script>
	...
	import {aihaozheUpdatePassword} from "@/api/aihaozhe/aihaozheApi";	import {chuangzuozheUpdatePassword} from "@/api/chuangzuozhe/chuangzuozheApi";	
				...
				this.$refs[editData].validate(valid => {
					if (valid) {
						if (this.editForm.newPassword === this.editForm.newPassword2) {
							...
							else if (this.userInfo.roles[0] == 'aihaozhe') {
								aihaozheUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
							...
							}else if (this.userInfo.roles[0] == 'chuangzuozhe') {
								chuangzuozheUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
